...
contract BnbProductionBondingCurveV1 is BnbProductionReentrancyGuardV1 {
    ...
    IBnbDexGraduationAdapterV1 public immutable graduationAdapter; uint256 public immutable minLpTimelock;
    ...
    event Graduated(address indexed adapter,address indexed pool,uint256 nativeAmount,uint256 tokenAmount,uint256 lpTimelock); event ExcessRecovered(address indexed recipient,uint256 nativeAmount,uint256 tokenAmount);

    constructor(string memory n,string memory s,uint256 supply,address tokenCreator,address platform,uint256 base,uint256 curveSlope,uint256 totalFeeBps,uint256 platformShareBps,address tokenAdmin,address adapter,uint256 lpTimelockMin) {
        ...
        if(base==0||totalFeeBps>MAX_FEE_BPS||platformShareBps>totalFeeBps||lpTimelockMin==0) revert InvalidConfig();
