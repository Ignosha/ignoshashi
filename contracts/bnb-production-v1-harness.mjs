...
class V1 {
  constructor({ supply=1000n, fee=100n, platform=50n, adapter=null, timelock=3600n }={}) {
    assert(supply > 0n && supply <= 10n**30n && fee <= 1000n && platform <= fee && adapter && timelock > 0n, "INVALID_CONFIG");
    this.supply=supply; this.fee=fee; this.platform=platform; this.adapter=adapter; this.minTimelock=timelock;
  ...
  request(p) { assert(this.state==='Active','ACTIVE'); assert(this.adapter,'ADAPTER_NOT_CONFIGURED'); assert(p.tokenAmount>0n&&p.nativeAmount>0n&&p.minTokenAmount<=p.tokenAmount&&p.minNativeAmount<=p.nativeAmount&&p.deadline>=0n,'INVALID_PARAMS'); assert(p.nonce===this.nonce,'INVALID_NONCE'); assert(p.lpTimelock>=this.minTimelock,'INVALID_TIMELOCK'); assert(p.nativeAmount<=this.reserve&&p.tokenAmount<=this.bal.get('curve'),'NOT_READY'); this.pending=p; this.state='Pending'; }
...
const p={tokenAmount:100n,nativeAmount:100n,minTokenAmount:99n,minNativeAmount:99n,deadline:0n,nonce:0n,lpTimelock:3600n};
