...
# Pool signing material is server-only and is intentionally not a prerequisite for
# frontend/static publication. The server validates each secret when its protected
# payout or graduation endpoint is requested (see server-db.ts). Never put key values
# in this script or in generated source/client bundles.
